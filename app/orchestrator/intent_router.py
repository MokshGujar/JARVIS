...
class IntentRouter:
    FILE_PATTERNS = (
        ...
        ("search", re.compile(r"^find\s+", re.I)),
    ...
    WHATSAPP_RE = re.compile(r"\bwhatsapp\b|\b(?:video\s+call|voice\s+call|call)\s+.+", re.I)
    BROWSER_PATTERNS = (
        ("browser_search", "search", re.compile(r"^(?:browser search|search browser|search in browser|google search|search google for)\s+.+", re.I)),
    ...
    def route(self, text: str) -> RouteDecision | None:
        ...
        if self.WHATSAPP_RE.search(command):
            return RouteDecision("whatsapp.communication", "whatsapp", "whatsapp", "communication")

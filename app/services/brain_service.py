...
class BrainService:
    ...
    def classify_primary(
        ...
        fast_category = self._fast_primary(msg)
        if fast_category:
            logger.info("[BRAIN-PRIMARY] %s -> %s (0 ms, rule-fast)", msg[:50], fast_category)
            return (fast_category, "rule-fast", 0)
    ...
    def classify_task(
        ...
        fast_decisions = self._fast_task_decisions(msg)
        if fast_decisions:
            self._last_task_decisions = fast_decisions
            task_types = [decision[0] for decision in fast_decisions]
            logger.info("[BRAIN-TASK] %s -> %s (0 ms, rule-fast)", msg[:50], fast_decisions)
            return (task_types, "rule-fast", 0)
...
Classify. Output EXACTLY ONE category name."""
    ...
    def _parse_task_decisions(self, raw_response: str) -> List[Tuple[str, str]]:
        ...
        decisions = []
        query_required_types = {"open", "play", "generate_image", "content", "google_search", "youtube_search"}
            ...
            for prefix in TASK_PREFIXES:
                if part_lower.startswith(prefix):
                    ...
                    if task_type in ("general", "realtime"):
                        continue
                    if task_type in query_required_types and not query:
                        matched = True
                        break
    ...
    def _fast_primary(self, msg: str) -> Optional[str]:
        m = " ".join((msg or "").strip().lower().split()).strip(" .!?")
        if not m:
            return "general"
        ...
        if m in {
            ...
            "thanks", "thank you", "bye", "goodbye", "uh", "um", "hmm",
        }:
            return "general"

        if any(x in m for x in [
            "what do you see", "what can you see", "what am i holding",
            "what is this", "describe this", "identify this", "what's in my hand",
            "look at this", "read this", "can you see", "check this out",
        ]):
            return "camera"

        action_patterns = [
            r"\bopen\b", r"\blaunch\b", r"\bgo to\b", r"\bvisit\b",
            r"\bplay\b", r"\bput on\b", r"\bgenerate\b", r"\bdraw\b",
            r"\bcreate\b.*\bimage\b", r"\bmake\b.*\bpicture\b",
            r"\bwrite\b", r"\bdraft\b", r"\bcompose\b",
            r"\bgoogle\b", r"\bsearch\b", r"\blook up\b",
        ]
        has_action = any(re.search(pattern, m) for pattern in action_patterns)
        has_info_request = any(x in m for x in [
            "tell me", "what is", "what are", "who is", "who are",
            "how does", "how do", "explain", "analysis", "fun fact",
            "give me details", "give me the answer",
        ])

        if has_action and has_info_request:
            return "mixed"
        if has_action:
            return "task"

        if any(x in m for x in [
            "latest", "current", "right now", "today", "news", "weather",
            "stock", "market price", "share price", "price of", "cost of",
            "score", "trending", "recent", "reviews", "best restaurants",
            "what happened",
        ]):
            return "realtime"
    ...
    def _fast_task_decisions(self, msg: str) -> List[Tuple[str, str]]:
        cleaned = self._strip_filler(msg or "")
        m = cleaned.lower()
        decisions: List[Tuple[str, str]] = []

        def clean_query(value: str) -> str:
            value = self._clean_fast_task_query(value)
            if self._looks_incomplete_task_query(value):
                return ""
            return value
        ...
        for segment in self._split_fast_task_segments(cleaned):
            segment = re.sub(
                r"^(?:uh|um|hmm|ah|oh)\s*,?\s*",
                "",
                segment.strip(),
                flags=re.I,
            )
            ...
            open_match = re.search(r"^(?:open|launch|go to|visit)\s+(?:the\s+)?(.+)$", segment, re.I)
            if open_match:
                query = clean_query(open_match.group(1))
                if query:
                    decisions.append(("open", query))
                continue

            play_match = re.search(r"^(?:play|put on)\s+(?:me\s+)?(.+)$", segment, re.I)
            if play_match:
                query = clean_query(play_match.group(1))
                if query:
                    decisions.append(("play", query))
                continue

            image_match = re.search(
                r"^(?:generate|draw|create|make)\s+(?:me\s+)?(?:an?\s+)?(?:image|picture|photo|logo)?\s*(?:of\s+)?(.+)$",
                segment,
                re.I,
            )
            if image_match and any(word in segment_lower for word in ["image", "picture", "photo", "draw", "generate", "logo"]):
                query = clean_query(image_match.group(1))
                if query:
                    decisions.append(("generate_image", query))
                continue

            content_match = re.search(r"^(?:write|draft|compose)\s+(?:me\s+)?(.+)$", segment, re.I)
            if content_match:
                query = clean_query(content_match.group(1))
                if query:
                    decisions.append(("content", query))
                continue

            youtube_match = re.search(r"^(?:search|find)\s+(?:youtube\s+)?(?:for\s+)?(.+?)\s+(?:on\s+)?youtube\b$", segment, re.I)
            if youtube_match:
                query = clean_query(youtube_match.group(1))
                if query:
                    decisions.append(("youtube_search", query))
                continue

            if re.search(r"\b(?:search|google|look up)\b", segment_lower):
                search_match = re.search(r"^(?:search(?: for)?|google|look up)\s+(.+)$", segment, re.I)
                query = clean_query(search_match.group(1)) if search_match else ""
                if query:
                    decisions.append(("google_search", query))
                continue

        deduped: List[Tuple[str, str]] = []
        seen = set()
        for task_type, query in decisions:
            key = (task_type, query.lower())
            if key not in seen:
                deduped.append((task_type, query))
                seen.add(key)

        return deduped

    def _split_fast_task_segments(self, text: str) -> List[str]:
        if not text:
            return []

        text = re.sub(r"^(?:image|picture|photo)\s+", "", text.strip(), flags=re.I)
        segments = re.split(
            r"\s+(?:and|also|then)\s+(?=(?:open|launch|go to|visit|play|put on|generate|draw|create|make|write|draft|compose|search|google|look up)\b)",
            text,
            flags=re.I,
        )
        return [segment.strip(" ,.!?\t\r\n") for segment in segments if segment.strip(" ,.!?\t\r\n")]

    def _clean_fast_task_query(self, value: str) -> str:
        value = (value or "").strip()
        value = re.sub(
            r"\s+\band\s+(?:tell|explain|what|who|how|give)\b.*$",
            "",
            value,
            flags=re.I,
        )
        value = re.sub(
            r"^(?:about\s+|for\s+|to\s+)?(?:the\s+|an?\s+|my\s+)?",
            "",
            value,
            flags=re.I,
        )
        value = re.sub(r"^(?:about\s*,\s*)", "", value, flags=re.I)
        value = re.sub(
            r"^\s*(?:can you|could you|would you|please|let'?s say)\s+",
            "",
            value,
            flags=re.I,
        )
    ...
    def _looks_incomplete_task_query(self, value: str) -> bool:
        if not value:
            return True

        lowered = value.lower().strip()
        if lowered in {"something", "anything", "that", "this", "it"}:
            return True
        ...
        trailing_words = {"for", "about", "to", "on", "regarding", "with", "of"}
        if tokens[-1] in trailing_words and len(tokens) <= 4:
            return True

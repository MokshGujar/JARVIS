    ...
    def __init__(self):
        ...
        if GROQ_API_KEYS:
            try:
                ...
                logger.info(
                    "[VISION] Groq vision initialized (%s)",
                    GROQ_VISION_MODEL,
                )

            except Exception as e:
                logger.warning(
                    "[VISION] Groq client init failed: %s",
                    e,
                )

        if not self._groq_client:
            logger.warning(
                "[VISION] No vision provider available. Set GROQ_API_KEY."
            )
    ...
    def describe_image(
        ...
        try:
            ...
            if raw_len > VISION_MAX_IMAGE_BYTES:
                logger.warning(
                    "[VISION] Image too large: %d bytes (max %d)",
                    raw_len,
                    VISION_MAX_IMAGE_BYTES,
                )
        ...
        logger.info(
            "[VISION] Analyzing image (%d bytes, %s)",
            raw_len,
            mime,
        )
    ...
    def _call_groq(self, messages: list) -> Optional[str]:
        try:
            ...
            if response.choices:
                ...
                if text:
                    logger.info(
                        "[VISION] Groq vision success (%d chars)",
                        len(text),
                    )
                    return text

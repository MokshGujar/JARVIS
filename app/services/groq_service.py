...
def _should_skip_retrieval(question: str) -> bool:
    text = " ".join((question or "").strip().lower().split()).strip(" .!?")
    if not text:
        return True
    ...
    return len(text.split()) <= 3 and not any(
        token in text
        for token in ("who", "what", "when", "where", "why", "how", "price", "stock")
    )
...
class GroqService:
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        logger.info(
            f"Initialized GroqService with {len(GROQ_API_KEYS)} API key(s) (primary-first fallback)"
        )
    ...
    def _invoke_llm(
        ...
        for j in range(n):
            ...
            try:
                ...
                if i > 0:
                    logger.info(
                        f"Fallback successful: API key #{i + 1}/{n} succeeded: {masked_key}"
                    )
                ...
                if _detect_repetition_loop(text):
                    logger.warning(
                        "[INVOKE] Repetition loop detected — truncating response (%d chars)",
                        len(text),
                    )
        ...
        for j in range(n):
            ...
            try:
                ...
                _log_timing(
                    ...
                    + (", TRUNCATED_REPETITION" if repetition_stopped else ""),
                )

                if i > 0 and chunk_count > 0:
                    logger.info(
                        f"Fallback successful: API key #{i + 1}/{n} streamed: {masked_key}"
                    )
    ...
    def _build_prompt_and_messages(
        ...
        try:
            if _should_skip_retrieval(question):
                logger.info("[CONTEXT] Skipped vector retrieval for lightweight message")
                context_docs = []
            else:
            ...
            if context_docs:
                ...
                logger.info(
                    "[CONTEXT] Retrieved %d chunks from sources: %s",
                    len(context_docs),
                    context_sources,
                )
            else:

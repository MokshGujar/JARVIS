...
class TaskExecutor:
    ...
    def __init__(self, groq_service=None, automation_service=None):
    ...
    def execute(
        ...
        for intent_type, payload in intents:
            ...
            if intent_type == INTENT_OPEN:
                self._forward_automation_intent(response, "open", payload)

            elif intent_type == INTENT_PLAY:
                self._forward_automation_intent(response, "play", payload)
                ...
                self._forward_automation_intent(response, "google search", payload)

            elif intent_type == INTENT_YOUTUBE_SEARCH:
                self._forward_automation_intent(response, "youtube search", payload)
                ...
                response.cam = {"action": "open"}
                response.text = (
                    "Opening your webcam. Once it's on, send your message again and I'll describe what I see."
                )
        ...
        try:
            ...
            logger.warning(
                "[TASK] Task execution timed out after %ds",
    ...
    def _forward_automation_intent(self, response: TaskResponse, prefix: str, payload: dict) -> None:
        if not self.automation_service:
            response.text = response.text or "Automation service is unavailable for that command."
            return
        message = (payload.get("message") or "").strip()
        if not message:
            target = (payload.get("query") or payload.get("url") or payload.get("target") or "").strip()
            message = f"{prefix} {target}".strip()
        if not message:
            response.text = response.text or "Tell me what automation action to run."
            return
        result = self.automation_service.execute(message)
        response.text = str(result.get("spoken_text") or result.get("message") or "Done.")
    ...
    def _do_generate_image(self, payload: dict) -> Optional[tuple]:
        ...
        if result:
            logger.info(
                "[TASK] Pollinations image downloaded in %.2fs",
            ...
            try:
                ...
                if resp.status_code == 200 and resp.content:
                    ...
                    if "image" in content_type or len(resp.content) > 1000:
                        logger.info(
                            "[TASK] Pollinations image fetched (%d bytes)",
                            len(resp.content),
                        )
                        return (api_url, resp.content)

                logger.warning(
                    "[TASK] Pollinations attempt %d failed: status=%d",
                    attempt + 1,
                    resp.status_code,
                )
                ...
                logger.warning(
                    "[TASK] Pollinations attempt %d failed: %s",
                    attempt + 1,
                    e,
                )
